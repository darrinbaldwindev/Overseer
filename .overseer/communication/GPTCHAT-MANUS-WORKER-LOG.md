...
## CORE-006 TRANSPORT-BOUNDARY REPAIR — 2026-09-01

STATUS: Local repair completed on a fresh canonical AgentOS snapshot. Canonical AgentOS already contains the Green Agent at `b46116cae7e32eb11ce596f29694c600686359b6`; this repair is local only and is not pushed.
CURRENT BLOCKER: No configured authenticated Manus task transport, callback/webhook, or correlated result adapter is available in AgentOS. A generic configurable Overseer.sh REST adapter exists, but it does not provide Manus task correlation, acknowledgement, consent handling, or independent verification.
ROOT CAUSE: The existing authority gate validated issuer and capabilities only, while the runner unconditionally invoked a worker after claim. There was no explicit local decision point for pre-authorized, confirmation-required, or prohibited execution.
WORK PERFORMED: Added provider-neutral `src/dispatch/consent.mjs`; guarded `src/dispatch/runner.mjs` so explicit `execution_authority: true` may execute, explicit false or `requires_confirmation` is durably blocked without worker invocation, and prohibited capabilities are durably blocked even when execution authority is true. Added `tests/consent-runner.test.mjs` and `docs/CONSENT_BOUNDARY.md`. Legacy tasks omitting explicit execution authority remain compatible as `legacy_pre_authorized`, with new tasks expected to be explicit.
COMMIT: Local AgentOS commit `d3250743b44204069e6f34cc8411ad9295f77136`, parent `b46116cae7e32eb11ce596f29694c600686359b6`; not pushed by this task.
TESTS: Focused command `node --test tests/consent-runner.test.mjs tests/dispatch-runner.test.mjs tests/dispatch-authority.test.mjs tests/dispatch-worker.test.mjs` — 11 passed, 0 failed. Full command `npm test` — 180 passed, 0 failed. `git diff --check` passed and changed-file inspection found no network calls, subprocess execution, credential handling, or protected-schedule mutation.
ACTUAL EXECUTION EVIDENCE: No live AgentOS → Manus transaction was created, dispatched, acknowledged, or independently verified. Local deterministic runner evidence proves only the consent boundary: pre-authorized work executes in the existing local runner; confirmation-required and prohibited work are blocked before worker invocation.
COMMUNICATION EVIDENCE: This append is the canonical coordination record. It does not prove a worker response or live transport. The generic Overseer.sh adapter remains provider-neutral but is not a Manus integration.
MANUS SAFETY/CONSENT FINDING: Pre-authorized worker execution is permitted only when the task explicitly carries `authority.execution_authority: true` and no prohibited capability. Confirmation-required execution includes `execution_authority: false` or `requires_confirmation: true` and remains blocked until a separate consent gate is satisfied. Prohibited execution includes repository writes, credential/permission changes, production mutation, provider or schedule activation, commercial actions, and destructive changes; confirmation cannot override prohibition. No Manus confirmation was bypassed or requested.
REMAINING OWNER ACTION: Authorize and configure a legitimate authenticated Manus API/Open App or first-party transport target, callback/webhook or bounded polling path, connector scope, and expiry/review window. Then authorize one bounded test transaction and independent verification. No credentials were changed by this task.
